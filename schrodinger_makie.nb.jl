...
heatmap(x, y, Vg,
	axis = (title="potenciál V_g", xlabel="x [nm]", ylabel="y [nm]", aspect=1),
...
# ╔═╡ 367b7aac-5086-4108-9e3a-9dff1861aa15
erg = reshape(eg, N, N, :)
...
heatmap(x, y, erg[:,:,kg],
	axis = (title="vlastní vektor $kg", xlabel="x [nm]", ylabel="y [nm]", aspect=1))

# ╔═╡ 415d1921-b87d-40ec-90e7-32e040a1ac83
begin
	local fig = Figure(size=(600,2000))
	for i in 1:12, j in 1:4
		k = (i - 1) * 4 + j
		ax = Axis(fig[i,j], aspect=1, title="$k",
			xticklabelsvisible=false, xticksvisible=false,
			yticklabelsvisible=false, yticksvisible=false)
		heatmap!(ax, x, y, erg[:,:,k])
	end
	fig
end
...
heatmap(x, y, Vb,
	axis = (title="potenciál V_b", xlabel="x [nm]", ylabel="y [nm]", aspect=1),
	colormap=Reverse(:viridis))
...
# ╔═╡ dcf97088-5af4-4ed9-b3d7-4a8794e9277b
erb = reshape(eb, N, N, :)
...
heatmap(x, y, erb[:,:,kb],
	axis = (title="vlastní vektor $kb", xlabel="x [nm]", ylabel="y [nm]", aspect=1))

# ╔═╡ f2d7ee1f-05a8-48ff-ab9e-572e21b43990
begin
	local fig = Figure(size=(600,2000))
	for i in 1:12, j in 1:4
		k = (i - 1) * 4 + j
...
manifest_format = "2.0"
project_hash = "7e66cd8ad149e8e26a34d3c32031fd3de96ab4b7"
